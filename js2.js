...

{
  ...
  const movSelected = await chooseMovie(["Movie A", "Movie B", "Movie C", "Movie D"]);
  console.log(movSelected);

  const showtime = await selectShowtime("Movie A");
  console.log(showtime);
  
  const payment = await makePayment();
  console.log(payment);
  // Concurrent Activities: Send email and SMS confirmations
  const [emailVerified, smsVerified] = await Promise.all([
      sendEmailConfirmation(),
      sendSMSConfirmation()
      ]);
    console.log(emailVerified);
    console.log(smsVerified);
