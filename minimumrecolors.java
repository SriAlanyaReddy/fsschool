...
class Solution{
    public static void main(String[] args){
       ...
       String s=sc.next();
       int n=sc.nextInt();
       int r=minimumrecolors(s,n);
       System.out.println(r);
    }
    public static int minimumrecolors(String s,int cons){
            int n=s.length();
            int min=Integer.MAX_VALUE;
            int c=0;
            for(int i=0;i<n-cons+1;i++){
                int countB=countchar(s,i,i+cons,'B');
                int countR=countchar(s,i,i+cons,'R');
                if(countB>countR){
                    c=c+countR;
                }
                min=Math.min(c,min);
    ...
    public static int countchar(String s,int l,int r,char c){
        int count=0;
        for(int i=l;i<r;i++){
